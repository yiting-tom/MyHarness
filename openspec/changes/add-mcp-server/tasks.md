...
- [ ] 3.1 `myharness/mcp/payload.py`：進度摘要的形狀與上限
      （規格：回傳給客戶端的內容受上限約束）
- [ ] 3.2 測試：事件數成長時，進度回應長度不成比例成長
- [ ] 3.3 結果回應 = executive summary + 章節價目表（est_tokens），不含全文
- [ ] 3.4 `analysis_drill` 一次一節且該節受 token 上限
- [ ] 3.5 待答問題在進度中呈現，且問題本身也受上限
