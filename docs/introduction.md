...
OpenRouter 上沒發生過 —— 這個模型話比較多。

（後來證明這句話的診斷是錯的：預算是累計量，視窗是單次長度，兩者不衝突。
真正的成因見下面〈Lane 預算〉。現象記錄保留原樣。）
...
很長一段時間這裡寫的是「`token_budget=60_000` 與 `max_model_len=65536` 只差 9%，
中間沒有餘裕」。**那是個類別錯誤。** 預算數的是一條 lane 累計送出的 token，
視窗管的是單一請求的長度，兩者量的不是同一件事。Golden #22 的 d1 花掉 63,752，
而它最後一個請求只帶 8,319——模型窗的 13%。沒有一次接近過。

真正的成因是上限在**數** token 而不是**算**價錢：多輪對話裡絕大多數 input 是
重送的前綴，而快取讀的實價是十分之一。詳見 `spikes/RESULTS.md` §78 次派工的成功率。
兩側都改成計價之後，那兩個 `budget_exceeded` 的派工落在預算的 27% 與 33%。

**只修了一半**：折扣只在宣告 `PROMPT_CACHING` 的 profile 上生效，而自架後端
的 usage 連 cache 欄位都不回報，所以它還是拿不到。
